...
Mezi tyto programy patří program UI, který spravuje OLED displej, přijímá od uživatele vstup rotačním enkodérem a je psaný v c++ pro jednodušší interakci s hardwarem, program web\_ui využívající runtime Node.js, ve kterém je nenáročné vytvořit http server, který je dostupný z lokální sítě a nakonec také program discord\_bot také využívající Node.js, který přijímá příkazy z chatovací aplikace discord a je přístupný i přes internet.
